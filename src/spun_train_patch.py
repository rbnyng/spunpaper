...
class ClimateExtractor:
    def __init__(self, climate_data_path, use_cache=True):
        ...
        self.cache_file = Path(paths.CLIMATE_METADATA_CACHE)
    ...
    def _generate_topography_layers(self):
        ...
        try:
            ...
            logging.error(
                "`richdem` not installed, so slope/aspect cannot be derived from the WorldClim DEM.."
            )
        except Exception as e:
...
class CombinedPatchClimateEvaluator:
    def __init__(self, climate_data_path, use_climate_cache, climate_features_cache_dir, soil_features_cache_dir,
                 ...
                 geotessera_patch_size: int = 3,   # the manuscript's window
                 geotessera_embeddings_dir: Optional[Union[str, Path]] = None,
                 geotessera_dataset_version: str = "v1",
                 geotessera_dataset_variant: str = "vultr",
                 geotessera_prune_tiles: bool = False,
        ...
        # Coerce here: argparse defaults reach us from paths.py as plain strings.
        self.soil_features_cache_dir = Path(soil_features_cache_dir) if soil_features_cache_dir else None
        if self.soil_features_cache_dir:
            logging.info(f"Using pre-processed soil feature cache at: {self.soil_features_cache_dir}")

        self.geotessera_year = geotessera_year
        self.geotessera_patch_size = geotessera_patch_size
        self.geotessera_embeddings_dir = geotessera_embeddings_dir
        self.geotessera_dataset_version = geotessera_dataset_version
        self.geotessera_dataset_variant = geotessera_dataset_variant
        self.geotessera_prune_tiles = geotessera_prune_tiles

        if satellite_source not in ("geotessera", "precomputed"):
            raise ValueError("satellite_source must be 'geotessera' or 'precomputed'")
        self.satellite_source = satellite_source

    @staticmethod
    def _index_by_id_segment(cache_dir: Path):
        index, ambiguous = {}, set()
        try:
            with os.scandir(cache_dir) as it:
                for e in it:
                    if not e.name.endswith(".npy"):
                        continue
                    for seg in e.name[:-4].split("_"):
                        if seg in index and index[seg] != e.path:
                            ambiguous.add(seg)
                        index[seg] = e.path
        except OSError:
            return {}
    ...
    def _load_precomputed_patches(self, cache_dir, sample_points):
        cache_dir = Path(cache_dir)
        out, skipped = {}, []
        ids = [str(s) for s in sample_points['sample_id']]

        # Fall back to segment indexing only when the plain convention finds
        # nothing, so the common case costs one stat() per sample as before.
        index = None
        if not any((cache_dir / f"{s}.npy").exists() for s in ids[:200]):
            index = self._index_by_id_segment(cache_dir)
            hits = sum(1 for s in ids if s in index)
            if hits:
                example = Path(index[next(s for s in ids if s in index)]).name
                logging.info(
                    "No {sample_id}.npy files in %s, but %d/%d sample IDs appear as a "
                    "filename segment (e.g. %s); loading those.",
                    cache_dir, hits, len(ids), example)
            else:
                index = None

        for sid in ids:
            f = Path(index[sid]) if index is not None and sid in index else cache_dir / f"{sid}.npy"
            if not f.exists():
                skipped.append({'id': sid, 'reason': 'No precomputed patch', 'stage': 'patch_load'})
                continue
            try:
                a = np.load(f)
            except Exception as e:
                skipped.append({'id': sid, 'reason': str(e), 'stage': 'patch_load'})
                continue
            if isinstance(a, np.ndarray) and a.ndim == 3 and np.isfinite(a).all():
                out[sid] = a.astype(np.float32, copy=False)
            else:
                skipped.append({'id': sid, 'reason': 'Invalid patch format', 'stage': 'patch_load'})
        return out, skipped


    @staticmethod
    def _fit_dim_reduction(X_satellite, method='none', n_components=None, random_state=42):
        if method == 'none' or X_satellite is None:
            names = [f"patch_{i}" for i in range(X_satellite.shape[1])] if X_satellite is not None else []
            return None, None, X_satellite, names

        scaler = StandardScaler()
        X_scaled = scaler.fit_transform(X_satellite)

        if n_components is None:
            # Auto-select number of components (sized off the TRAIN set only).
            if method == 'pca':
        ...
        n_samples, n_features = X_satellite.shape
        requested = n_components
        n_components = int(min(n_components, n_features, max(2, n_samples - 2)))
        if n_components != requested:
            logging.warning(
                f"{method.upper()} components reduced {requested} -> {n_components} "
                f"to fit {n_samples} samples x {n_features} features.")
        ...
        if method == 'pca':
            ...
            feature_names = [f"pca_{i+1}" for i in range(model.n_components_)]
            ...
            X_reduced = model.fit_transform(X_scaled)
            feature_names = [f"umap_{i+1}" for i in range(X_reduced.shape[1])]
        else:
            raise ValueError(f"Unknown dimensionality reduction method: {method}")

        return scaler, model, X_reduced, feature_names

    @staticmethod
    def _transform_dim_reduction(X_satellite, method, scaler, model):
        if method == 'none' or X_satellite is None:
            return X_satellite
        if scaler is None or model is None:
            raise ValueError("Models not fitted. Cannot transform held-out data.")
        return model.transform(scaler.transform(X_satellite))

    def fit_satellite_reduction(self, X, dr_config):
        dr_config = dr_config or {}
        n_sat = dr_config.get('n_satellite_raw', 0)
        method = dr_config.get('method', 'none')
        if not n_sat or method == 'none':
            self.scaler, self.dim_reduction_model = None, None
            return X
        scaler, model, X_sat_red, _ = self._fit_dim_reduction(
            X[:, :n_sat], method=method,
            n_components=dr_config.get('n_components'),
            random_state=dr_config.get('random_state', 42),
        )
        self.scaler, self.dim_reduction_model = scaler, model
        logging.info(f"Satellite block reduced {n_sat} -> {X_sat_red.shape[1]} dims "
                     f"(fit on all {len(X)} rows; inference workflow).")
        return np.hstack([X_sat_red, X[:, n_sat:]])
    ...
    def prepare_dataset(self,
        ...
        if worldcover_features_path is not None:
            worldcover_features_path = Path(worldcover_features_path)
        ...
        if use_satellite:
            if not representations_dir_path:
                logging.error("use_satellite is True, but a representations/cache dir was not provided.")
                return None, None, [], [], None, None, None
            cache_dir = Path(representations_dir_path)
            sample_points = current_merged_df[['sample_id', 'latitude', 'longitude']].drop_duplicates('sample_id')

            if self.satellite_source == "precomputed":
                logging.info(f"Loading precomputed satellite patches from: {cache_dir}")
                all_repr_dict, skipped_locations_from_npy = self._load_precomputed_patches(
                    cache_dir, sample_points)
            else:
                cache_dir.mkdir(parents=True, exist_ok=True)
                logging.info(
                    f"Fetching Tessera embeddings via geotessera "
                    f"(year={self.geotessera_year}, patch_size={self.geotessera_patch_size}, "
                    f"version={self.geotessera_dataset_version}/{self.geotessera_dataset_variant}); "
                    f"per-sample cache at: {cache_dir}"
                )

                try:
                    _dg = Path(__file__).resolve().parent.parent / "data_generation"
                    if _dg.is_dir() and str(_dg) not in sys.path:
                        sys.path.insert(0, str(_dg))
                    from geotessera_embeddings import GeoTesseraPatchExtractor
                except ImportError as e:
                    logging.error(
                        f"Could not import geotessera_embeddings ({e}). Fetching embeddings "
                        f"needs data_generation/ and geotessera (Python >= 3.12). To reproduce "
                        f"from the deposited cache instead, use --satellite-source precomputed.")
                    return None, None, [], [{'id': 'N/A', 'reason': str(e), 'stage': 'patch_load'}], None, None, None

                extractor = GeoTesseraPatchExtractor(
                    year=self.geotessera_year,
                    patch_size=self.geotessera_patch_size,
                    cache_dir=cache_dir,
                    embeddings_dir=self.geotessera_embeddings_dir,
                    dataset_version=self.geotessera_dataset_version,
                    dataset_variant=self.geotessera_dataset_variant,
                )

                # Fetching is grouped by 0.1-degree tile: one download serves every
                # sample inside a tile.
                all_repr_dict = extractor.extract_for_dataframe(
                    sample_points,
                    id_col='sample_id', lat_col='latitude', lon_col='longitude',
                    prune_tiles=self.geotessera_prune_tiles,
                )
                skipped_locations_from_npy = []
                # (the precomputed loader already records its own misses)
                for sid in sample_points['sample_id'].astype(str):
                    if sid not in all_repr_dict:
                        skipped_locations_from_npy.append(
                            {'id': sid, 'reason': 'No Tessera coverage', 'stage': 'patch_load'})

            for sid, patch in list(all_repr_dict.items()):
                if not (isinstance(patch, np.ndarray) and patch.ndim == 3):
                    skipped_locations_from_npy.append(
                        {'id': sid, 'reason': 'Invalid patch format', 'stage': 'patch_load'})
                    all_repr_dict.pop(sid, None)
                    continue
                if patch_dims is None:
                    patch_dims = tuple(patch.shape)
                    logging.info(f"Satellite patch dimensions (P1, P2, C): {patch_dims}")
                elif tuple(patch.shape) != tuple(patch_dims):
                    skipped_locations_from_npy.append(
                        {'id': sid, 'reason': 'Inconsistent patch dimensions', 'stage': 'patch_load'})
                    all_repr_dict.pop(sid, None)

            all_skipped_locations.extend(skipped_locations_from_npy)
            if not all_repr_dict:
                logging.error("No Tessera representations could be fetched for any sample.")
                return None, None, [], all_skipped_locations, None, None, None

            repr_df = pd.DataFrame([{'sample_id': sid, 'representation_patch': patch} for sid, patch in all_repr_dict.items()])
            logging.info(f"Total unique Tessera patch representations collected: {len(repr_df)}")
            ...
            if current_merged_df.empty:
                ...
                return None, None, [], all_skipped_locations, patch_dims, None, None
            ...
            if np.isnan(X_patch_flat).any():
                logging.warning("NaNs found in features. Imputing with column means.")
                X_patch_flat = np.nan_to_num(X_patch_flat, nan=np.nanmean(X_patch_flat, axis=0))

            patch_feature_names_list = [f"patch_{i}" for i in range(X_patch_flat.shape[1])]
        
        if use_climate:
            extractor_ready = bool(self.climate_extractor) and self.climate_extractor.data_loaded
            if not extractor_ready:
                ids_needed = current_merged_df['sample_id'].astype(str)
                cached = (self.climate_features_cache_dir is not None and all(
                    (self.climate_features_cache_dir / f"{sid}.npy").exists() for sid in ids_needed))
                if not cached:
                    logging.error(
                        "ClimateExtractor not ready and the climate feature cache does not "
                        "cover every sample. Set climate_data_path to the WorldClim "
                        "directory, or populate the cache first.")
                    return None, None, [], all_skipped_locations, patch_dims, None, None
                logging.info("ClimateExtractor unavailable, but every sample is in the "
                             "climate feature cache; proceeding from cache.")
            ...
            for _, row in tqdm(points_for_climate.iterrows(), total=len(points_for_climate), desc="Climate Features"):
                ...
                try:
                    ...
                    elif not extractor_ready:
                        climate_feats_dict = None
                    else:
            ...
            if not all_climate_features:
                ...
                if not use_satellite: return None, None, [], all_skipped_locations, patch_dims, None, None
                logging.warning("Proceeding with SSL data as no climate features were found.")
            else:
        ...
        if X_patch_flat is not None and use_satellite:
            ...
            n_satellite_raw = X_patch_flat.shape[1]
        ...
        if not final_X_list:
            logging.info("No feature sources requested; building the coordinates-only "
        ...
        if use_satellite and X_patch_flat is not None:
            logging.info(f"  Satellite features (raw, reduced later on train split): {n_satellite_raw}")
        ...
        dr_config = {
            'n_satellite_raw': n_satellite_raw,
            'method': satellite_dim_reduction,
            'n_components': dim_reduction_components,
            'random_state': random_state,
        }

        return (X_combined, y_combined, processed_locations, all_skipped_locations,
                patch_dims, all_feature_names, dr_config)
    ...
    def train_and_evaluate(self, X, y, location_info, all_feature_names, random_seed=42, model_name='rf', model_params=None,
        ...
        # 0.125 of the 80% train+val block leaves 70/10/20 overall.
        ...
        dr_config = dr_config or {}
        n_sat = dr_config.get('n_satellite_raw', 0)
        dr_method = dr_config.get('method', 'none')
        feature_names = list(all_feature_names) if all_feature_names is not None else None

        if n_sat > 0 and dr_method != 'none':
            def _split_sat(A):
                return A[:, :n_sat], A[:, n_sat:]

            Xtr_sat, Xtr_rest = _split_sat(X_train)
            Xval_sat, Xval_rest = _split_sat(X_val)
            Xte_sat, Xte_rest = _split_sat(X_test)

            scaler, dr_model, Xtr_sat_red, sat_names = self._fit_dim_reduction(
                Xtr_sat, method=dr_method,
                n_components=dr_config.get('n_components'),
                random_state=dr_config.get('random_state', random_seed),
            )
            Xval_sat_red = self._transform_dim_reduction(Xval_sat, dr_method, scaler, dr_model)
            Xte_sat_red = self._transform_dim_reduction(Xte_sat, dr_method, scaler, dr_model)

            # Persist the fitted transform. Consumers that pickle the evaluator and
            # replay it over a raster (10m_map_masked, fig1_plots,
            # uk_national_parks_inference) read these two attributes back.
            self.scaler, self.dim_reduction_model = scaler, dr_model

            X_train = np.hstack([Xtr_sat_red, Xtr_rest])
            X_val = np.hstack([Xval_sat_red, Xval_rest])
            X_test = np.hstack([Xte_sat_red, Xte_rest])

            # The raw patch_* names occupied the first n_sat slots.
            if feature_names is not None and len(feature_names) >= n_sat:
                feature_names = list(sat_names) + list(feature_names[n_sat:])

            logging.info(f"Satellite block reduced {n_sat} -> {Xtr_sat_red.shape[1]} dims "
                         f"(fit on {len(X_train)} train rows only).")
        ...
        try:
            device = model_params.pop('device', None) or _default_device()

            if model_name == 'lightgbm':
                lgbm_defaults = {'random_state': random_seed, 'n_estimators': 1000, 'learning_rate': 0.05, 'n_jobs': -1}
                model = lgb.LGBMRegressor(**{**lgbm_defaults, **model_params}, device=device)
                lgbm_fit(model, X_train, y_train, eval_X=X_val, eval_y=y_val,
                         stopping_rounds=15)

            elif model_name == 'xgboost':
                xgb_defaults = {'random_state': random_seed, 'n_estimators': 1000, 'learning_rate': 0.05, 'n_jobs': -1}
                try:
                    model = xgb.XGBRegressor(**{**xgb_defaults, **model_params},
                                             device=device, early_stopping_rounds=15)
                    model.fit(X_train, y_train, verbose=False, eval_set=[(X_val, y_val)])
                except TypeError:
                    model = xgb.XGBRegressor(**{**xgb_defaults, **model_params}, device=device)
                    model.fit(X_train, y_train, verbose=False, eval_set=[(X_val, y_val)],
            ...
            # R2 on the central 98% of the data, excluding extreme outliers.
...
def reduce_satellite_block(dr_config, X_train, *X_others):
    dr_config = dr_config or {}
    n_sat = dr_config.get('n_satellite_raw', 0)
    method = dr_config.get('method', 'none')
    if not n_sat or method == 'none':
        return (X_train, *X_others) if X_others else X_train

    scaler, model, Xtr_sat_red, _ = CombinedPatchClimateEvaluator._fit_dim_reduction(
        X_train[:, :n_sat], method=method,
        n_components=dr_config.get('n_components'),
        random_state=dr_config.get('random_state', 42),
    )
    out = [np.hstack([Xtr_sat_red, X_train[:, n_sat:]])]
    for Xo in X_others:
        Xo_sat_red = CombinedPatchClimateEvaluator._transform_dim_reduction(
            Xo[:, :n_sat], method, scaler, model)
        out.append(np.hstack([Xo_sat_red, Xo[:, n_sat:]]))
    return tuple(out) if X_others else out[0]


def _default_device():
    try:
        import torch  # optional dependency; only used for the probe
...
def lgbm_fit(model, X, y, eval_X=None, eval_y=None, stopping_rounds=None, **kwargs):
    if eval_X is not None:
        if "eval_X" in inspect.signature(model.fit).parameters:
            kwargs.update(eval_X=eval_X, eval_y=eval_y)
        else:
            kwargs["eval_set"] = [(eval_X, eval_y)]
        if stopping_rounds:
            kwargs.setdefault("callbacks", []).append(
                lgb.early_stopping(stopping_rounds=stopping_rounds, verbose=False))
    model.fit(X, y, **kwargs)
    return model


def _data_dir(run_dir):
    d = Path(run_dir) / "data"
...
def analyze_and_save_feature_importance(all_run_results, all_feature_names, run_dir):
    ...
    widths = [len(r['feature_importance']) for r in successful_runs]
    modal_width = max(set(widths), key=widths.count)
    kept = [r for r in successful_runs if len(r['feature_importance']) == modal_width]
    if len(kept) < len(successful_runs):
        logging.warning(
            f"Feature-importance width varied across runs; averaging the {len(kept)}/"
            f"{len(successful_runs)} runs with the modal width ({modal_width}).")
    successful_runs = kept

    names = next((r.get('feature_names') for r in successful_runs if r.get('feature_names')), None)
    if not names or len(names) != modal_width:
        names = all_feature_names if (all_feature_names and len(all_feature_names) == modal_width) \
            else [f"f_{i}" for i in range(modal_width)]
    all_feature_names = list(names)
    ...
    # Persist the numbers both figures below are drawn from.
    results_io.dump(
        _data_dir(run_dir), "feature_importance",
        stats={"n_successful_runs": len(successful_runs),
               "n_features": len(all_feature_names)},
        arrays={"per_run_importance": np.array([r['feature_importance'] for r in successful_runs]),
                "avg_importance": avg_importances},
        tables={"importance": importance_df,
                "by_category": category_importance.reset_index()},
    )
...
def analyze_and_save_error_analysis(run_result, run_dir):
    ...
    results_io.dump(
        _data_dir(run_dir), "error_analysis",
        stats={"n_test": int(len(y_test)),
               "r2": float(run_result.get('test_stats', {}).get('r2', float('nan')))},
        arrays={"y_test": y_test, "test_pred": test_pred},
        tables={"errors": error_df},
    )
...
def analyze_and_save_spatial_error_maps(run_result, run_dir):
    ...
    # Ensure info_test is a df, as it might be a list of dicts
    ...
    results_io.dump(
        _data_dir(run_dir), "spatial_errors",
        stats={"n_points": int(len(error_df))},
        tables={"points": error_df},
    )
...
def main_evaluation(args):
    ...
    # Spot-testing filter: restrict the run to a few bounding boxes.
    ...
    if USE_BBOX_FILTER and BOUNDING_BOXES:
        ...
        for box in BOUNDING_BOXES:
            ...
            combined_mask |= box_mask # a point matches if it is in ANY box
    ...
    evaluator = CombinedPatchClimateEvaluator(
        ...
        geotessera_year=args.year,
        geotessera_patch_size=args.patch_size,
        geotessera_embeddings_dir=args.geotessera_embeddings_dir,
        geotessera_dataset_version=args.dataset_version,
        geotessera_dataset_variant=args.dataset_variant,
        geotessera_prune_tiles=args.prune_tiles,
    ...
    X_combined, y_combined, locations, _, _, f_names, dr_config = evaluator.prepare_dataset(
        biodiversity_df, 
    ...
    logging.info(f"Dataset ready with {X_combined.shape[1]} raw features "
                 f"(satellite reduced per run on the train split).")

    # The reducer (UMAP/PCA) is refit on each run's training split and applied
    # to validation and test, so held-out rows never enter its basis.
    all_run_results = []
    ...
    results_io.dump(
        _data_dir(run_dir), "all_run_results",
        stats={"n_runs": len(all_run_results),
               "n_successful": sum(1 for r in all_run_results if 'error' not in r),
               "model": MODEL_TO_USE,
               "dim_reduction": SATELLITE_DIM_REDUCTION,
               "n_samples": int(len(X_combined)),
               "n_features": int(X_combined.shape[1]),
               "runs": all_run_results},
        arrays={"X_combined": X_combined, "y_combined": y_combined},
        tables={"feature_names": pd.DataFrame({"feature": f_names}),
                "locations": pd.DataFrame(locations)},
        meta={"year": args.year, "patch_size": args.patch_size,
              "representations_dir": str(REPRESENTATIONS_DIR_PATH),
              "use_satellite": USE_SATELLITE_DATA, "use_climate": USE_CLIMATE_DATA,
              "use_soil": USE_SOIL_DATA, "use_worldcover": USE_WORLDCOVER_DATA},
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--year', type=int, default=2024, help='Tessera embedding year to fetch (2017-2025).')
    parser.add_argument('--satellite-source', default='geotessera',
                        choices=['geotessera', 'precomputed'],
                        help="'precomputed' reads {sample_id}.npy straight from the "
                             "representations directory with no fetching -- the right "
                             "choice for a deposited cache, and for any feature set "
                             "that is not Tessera (e.g. the spectral baseline).")
    parser.add_argument('--patch-size', type=int, default=int(os.environ.get('PATCH_SIZE', 3)),
                        help="Tessera patch size in 10 m pixels. 3 is the manuscript's "
                             "window and the default everywhere else; override via the "
                             "PATCH_SIZE environment variable or config.env.")
    parser.add_argument('--dataset-version', type=str, default='v1', help='geotessera dataset version (e.g. v1, v1.1).')
    parser.add_argument('--dataset-variant', type=str, default='vultr', help='geotessera dataset variant (e.g. vultr, cambridge).')
    parser.add_argument('--geotessera-embeddings-dir', type=str, default=None,
                        help='Directory for geotessera tile downloads (default <representations_dir>/tessera_tiles).')
    parser.add_argument('--prune-tiles', action='store_true',
                        help='Delete each ~110 MB Tessera tile once its samples are extracted.')
    ...
    parser.add_argument('--biome-cache-dir', type=str, default=paths.ECOREGIONS_CACHE, help='Directory to cache RESOLVE Ecoregions data')
    ...
    parser.add_argument('--biodiversity_csvs', nargs='+', default=[paths.ECM_EUROPE_CSV, paths.ECM_ASIA_CSV])
    parser.add_argument('--representations_dir', type=str, default=paths.REPRESENTATIONS_DIR,
                        help='Directory used as the per-sample Tessera patch cache ({sample_id}.npy). '
                             'Created if missing; reused across runs.')
    parser.add_argument('--climate_data_dir', type=str, default=paths.WORLDCLIM_DIR)
    parser.add_argument('--climate_cache_dir', type=str, default=paths.CLIMATE_CACHE_DIR)
    parser.add_argument('--soil_cache_dir', type=str, default=paths.SOIL_CACHE_DIR)
    parser.add_argument('--worldcover_path', type=str, default=paths.WORLDCOVER_CSV)
    parser.add_argument('--results_dir', type=str, default=paths.RESULTS_DIR,
                        help='Where run directories are written. Defaults to RESULTS_DIR '
                             'from config.env, which is where the aggregation and plotting '
                             'scripts look; overriding it here decouples the two.')
