...
from spun_train_patch import (
    CombinedPatchClimateEvaluator, filter_by_biome, lgb, _default_device,
)
...
def generate_samples_map(config: AssetConfig):
    ...
    gdf.plot(ax=ax, marker='o', color='crimson', markersize=5, alpha=0.6)
...
def generate_performance_scatterplot(config: AssetConfig):
    ...
    model = lgb.LGBMRegressor(random_state=42, n_estimators=1000, learning_rate=0.05, n_jobs=-1, device=_default_device())
